...
import interfaces.ModuloCliente.HandlerConfirmAlterCliente;
...
import interfaces.ModuloProduto.HandlerConfirmAlterProduto;
	...
	private static class ChooseYes implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if(caller instanceof CadastraVendas) {
				...
				} else if(subCaller instanceof HandlerConfirmAlterProduto) {
					((ModuloProduto) caller).atualizaProduto(true);
				}
			} else if(caller instanceof ModuloFornecedor) {
				if(subCaller instanceof HandlerAddFornecedor) {
					((ModuloFornecedor) caller).incluiFornecedor(true);
				...
				} else if(subCaller instanceof HandlerConfirmAlterCliente) {
					((ModuloCliente) caller).atualizaCliente(true);
				}
			}
			source.dispose();
		}
		...
		public void actionPerformed(ActionEvent e) {
			if(caller instanceof CadastraVendas) {
				...
				if(subCaller instanceof HandlerAddProduto) {
					...
					((ModuloProduto) caller).atualizaProduto(false);
				}
			} else if(caller instanceof ModuloFornecedor) {
				if(subCaller instanceof HandlerAddFornecedor) {
				...
				if(subCaller instanceof HandlerAddCliente) {
					...
					((ModuloCliente) caller).atualizaCliente(false);
				}
			}
			source.dispose();
		}
